...
function ProficiencyList({ data, sortKey, filter }: { data: WordProficiency[]; sortKey: SortKey; filter: Filter }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    return (
      b.correctCount + (b.halfCorrectCount ?? 0) + b.incorrectCount - (a.correctCount + (a.halfCorrectCount ?? 0) + a.incorrectCount)
    );
  });
  ...
  return (
    <>
      {sorted.map((p) => {
        ...
        return (
          <div key={p.word} className={`flex items-center justify-between px-3 py-1.5 rounded border text-sm ${getLevelBg(level)}`}>
            <span className="font-medium truncate mr-2">{p.word}</span>
            <div className="flex items-center gap-2 shrink-0">
              <span className={`text-xs font-bold ${getLevelColor(level)}`}>{getLevelLabel(level)}</span>
              <span className="text-xs text-gray-500">{score}% ({p.correctCount}/{half > 0 ? `${half}/` : ""}{p.incorrectCount})</span>
            </div>
          </div>
        );
      })}
