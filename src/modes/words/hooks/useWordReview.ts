...
type UseWordReviewOptions = {
  onRemoveIncorrectWord?: (word: string, typed: string) => void;
};

export function useWordReview(options: UseWordReviewOptions = {}) {
  const { onRemoveIncorrectWord } = options;
  ...
  const handleReviewSubmit = useCallback(
    (input: string): boolean => {
      ...
      if (isCorrect && reviewType === "primary") {
        ...
        onRemoveIncorrectWord?.(targetWord.word, targetWord.typed);
      }
    ...
    [currentReviewIndex, isReviewActive, onRemoveIncorrectWord, reviewType, reviewWords]
  );
